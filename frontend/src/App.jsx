...
function App() {
  ...
  const [form, setForm] = useState({ title: "", start: "", end: "", priority: "medium" });
  ...
  const [accountForm, setAccountForm] = useState({ name: "", age: "", date_of_birth: "", gender: "", use_case: "student", preferred_focus_time: "", planning_style: "", daily_screen_time: "", preferred_task_difficulty: "" });
  const [profileSaving, setProfileSaving] = useState(false);
  const [profileEditing, setProfileEditing] = useState(false);
  const [routineDate, setRoutineDate] = useState(() => new Date().toISOString().slice(0, 10));
  const [routineTimes, setRoutineTimes] = useState({ wake_up: "", breakfast: "", lunch: "", exercise: "", dinner: "", wind_down: "", sleep: "" });
  ...
  const loadRoutine = async (selectedDate = routineDate) => {
    try {
      ...
      const nextTimes = { wake_up: "", breakfast: "", lunch: "", exercise: "", dinner: "", wind_down: "", sleep: "" };
  ...
  const createTask = async (event) => {
    ...
    try {
      const result = await axios.post(`${API_BASE}/tasks/`, null, { params: { title: form.title, scheduled_time: start.toISOString(), expected_end_time: end.toISOString(), priority: form.priority }, ...headers });
      const task = result.data;
      setTasks((previous) => [task, ...previous]);
      scheduleNativeNotifications(task);
      setForm({ title: "", start: "", end: "", priority: "medium" });
  ...
  const applyPreset = (title, minutes) => {
    ...
    setForm({ title, start: dateTimeLocal(start), end: dateTimeLocal(end), priority: "medium" });
    setPage("schedule");
  };
  ...
  const nav = [["dashboard", "Overview", "⌂"], ["schedule", "Plan", "+"], ["routine", "Routine", "◔"], ["history", "History", "◷"], ["settings", "Settings", "⚙"]];

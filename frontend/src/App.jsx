...
function App() {
  ...
  const openTask = (task) => {
    ...
    setResponse(task.status === "completed" ? "pending" : "completed");
    setNotes("");
  ...
  const completeDashboardItem = async (item) => {
    ...
    const userResponse = item.status === "completed" ? "pending" : "completed";
    try {
      const result = await axios.put(`${API_BASE}/tasks/${item.id}/respond`, null, {
        params: { user_response: userResponse, notes: "" }, ...headers,
      });
      setTasks((previous) => previous.map((task) => task.id === item.id ? result.data.task : task));
    } catch (error) {
      setNotice(error.response?.data?.detail || "Could not update this task.");
    }
  };
  ...
  return <div className="app-shell">
    ...
    <main className="workspace"><header className="topbar"><div><p className="eyebrow">{page === "dashboard" ? "GOOD TO SEE YOU" : "YOUR PERSONAL SPACE"}</p><h1>{page === "dashboard" ? `Hello${user?.name ? `, ${user.name}` : ""}.` : page[0].toUpperCase() + page.slice(1)}</h1></div><div className="topbar-actions"><button className="secondary-button" onClick={openWrapUp}>Wrap up day</button><button className="notification-toggle" onClick={() => setMuted(!muted)}>{muted ? "Notifications off" : "Notifications on"}</button></div></header>{notice && <div className="notice success app-notice">{notice}<button onClick={() => setNotice("")}>×</button></div>}
      {page === "dashboard" && <>
        <section className="stats-grid"><article><span>ACTIVE PLANS</span><strong>{activeTasks.length}</strong><small>Ready for your attention</small></article><article><span>COMPLETED</span><strong>{completedCount}</strong><small>Tasks in your history</small></article><article><span>FOLLOW-THROUGH</span><strong>{completionRate}%</strong><small>Of logged tasks</small></article></section>
        <section className="content-grid"><article className="panel wide"><div className="panel-heading"><div><p className="eyebrow">{showingUpcomingSchedule ? "UPCOMING PLAN" : "NEXT UP"}</p><h2>{showingUpcomingSchedule ? "Your upcoming schedule" : "Your schedule"}</h2></div><button className="secondary-button" onClick={() => setPage("schedule")}>Plan a task</button></div>{overviewSchedule.length ? <div className="daily-checklist">{overviewSchedule.map((item) => <DailyScheduleItem key={`${item.type}-${item.id}`} item={item} onToggle={() => completeDashboardItem(item)} onDetail={() => openTaskDetail(item.task)} onUpdate={() => openTask(item.task)} />)}</div> : <EmptyState title="A clear day starts with one plan." action={hasPriorActivity ? "Schedule today's first task" : "Schedule your first task"} onClick={() => setPage("schedule")} />}</article><article className="panel coach-card"><p className="eyebrow">YOUR SECOND MIND</p><h2>Best next move</h2><p>{secondMind?.answer || coach}</p><hr /><p className="insight"><b>{secondMind?.suggested_time ? `Suggested time · ${secondMind.suggested_time}` : "Suggested next step"}</b>{insight}</p>{secondMind?.data_mode === "bootstrap" && <p className="ai-status">Starter guidance · personalized as you add routine and outcomes</p>}{secondMind?.data_mode === "blended" && <p className="ai-status">Learning from your routine · becoming more personal</p>}<button className="secondary-button coach-link" onClick={() => setPage("coach")}>Ask Orbit</button></article></section>
      </>}

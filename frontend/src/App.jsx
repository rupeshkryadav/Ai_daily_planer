...
function App() {
  ...
  const [form, setForm] = useState({ title: "", start: "", durationHours: "", durationMinutes: "", priority: "medium" });
  ...
  const createTask = async (event) => {
    event.preventDefault();
    if (!form.title || !form.start) return setNotice("Add a title and start time.");
    ...
    try {
      ...
      setForm({ title: "", start: "", durationHours: "", durationMinutes: "", priority: "medium" });
  ...
  const applyPreset = (title, minutes) => {
    ...
    setForm({ title, start: dateTimeLocal(start), durationHours: String(Math.floor(minutes / 60)), durationMinutes: String(minutes % 60), priority: "medium" });
    setPage("schedule");
  };

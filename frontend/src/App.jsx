...
function App() {
  ...
  const openTaskDetail = async (task) => {
    ...
    try {
      ...
      setTaskDetail({ task, energy_required: task.current_energy_level || 5, energy_available: dailyCheckIn?.energy_level || 5, stress_level: dailyCheckIn?.stress_level || 5, completion_probability: 65, recommendation: "Orbit is still learning your current workload. Keep this task focused and review it after completion." });
    }
  };
  ...
  const updateTask = async () => {
    ...
    try {
      if (editDifficulty && editDifficulty !== selectedTask.task_difficulty) {
        const edited = await axios.put(`${API_BASE}/tasks/${selectedTask.id}`, { task_difficulty: editDifficulty }, headers);
        setTasks((previous) => previous.map((task) => task.id === edited.data.id ? edited.data : task));
      }
